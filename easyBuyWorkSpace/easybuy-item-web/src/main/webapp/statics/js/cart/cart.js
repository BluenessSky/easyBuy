...
function addCartByParam(entityId,quantity){
	window.location.href="http://localhost:8084/cart/addToCart?entityId="+entityId+"&quantity="+quantity;
    //添加到购物车
}
/**
 * 刷新购物车 searchBar DIV
...
function refreshCart(){
    $.ajax({
        url: "http://localhost:8083/cart/refreshCart",
        method: "post",
        dataType:"jsonp",
        data: null,
        success: function (data) {
            ...
            for(var i = 0; i < data.items.length; i++){
            	...
            	$li.append("<div class='price' style='margin-bottom: 20px;border-bottom: 1px solid #e2e2e2;'><font color='#ff4e00'>￥"+data.items[i].price+"</font> X"+data.items[i].stock+"</div>");
            }
            //购物车中的商品数量
